    ...
    {products.map((result, i) => (
      <tr key={i}>
        ...
        <td>
          ...
          <button className="btn btn-light" value="Submit" onClick={(e) => deleteProduct(e, result.ProductName)}>X
            REMOVE
          </button>
          <button className="btn btn-light" value="Reset">SAVE FOR LATER</button>
        </td>
        <td>S</td>
        <td><input type="number" className="form-control form-control--small" defaultValue={result.quantity}
                   onChange={(e) => onChange(e.target.value, result.ProductName, i)}/></td>
